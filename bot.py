...
async def follow(message) -> Iterator[str]:
    ...
    async with aiofiles.open(bot_data.LOGPATH, mode='r') as file: # async with aiofiles fixes Discord.gateway warning "Shard ID None heartbeat blocked for more than 10 seconds."
...
async def log_handler(message):
        ...
        async for line in task:
            # Send latest Death to Discord channel
